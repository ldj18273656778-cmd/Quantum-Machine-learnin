...
import sys
import time
from typing import Any, Callable, TextIO, assert_never
...
from task2_code_auto.loss_registry import active_loss_breakdown, loss_function_uses_superoperator, set_active_loss_function
from task2_code_auto.module_e_training import (
    AdamConfig,
    adam_optimize,
...
StepCallback = Callable[[int, NDArray[np.float64], float], None]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--gradient-backend", choices=["finite-difference", "jax"], default="finite-difference")
    parser.add_argument("--jax-memory-mode", choices=["standard", "rematerialized"], default="standard")
    ...
    parser.add_argument(
        ...
        type=_memory_telemetry_path,
        default=None,
        help="append per-step JAX memory telemetry JSONL to PATH",
    )
    args = parser.parse_args()
    if args.gradient_backend != "jax" and args.jax_memory_mode != "standard":
        parser.error("--jax-memory-mode rematerialized requires --gradient-backend jax")
    return args


def _memory_telemetry_path(value: str) -> Path:
    path = Path(value)
    parent = path.parent if path.parent != Path("") else Path(".")
    if not parent.exists():
        raise argparse.ArgumentTypeError(f"memory telemetry parent directory does not exist: {parent}")
    if not parent.is_dir():
        raise argparse.ArgumentTypeError(f"memory telemetry parent path is not a directory: {parent}")
    if path.exists() and path.is_dir():
        raise argparse.ArgumentTypeError(f"memory telemetry path is a directory: {path}")
    return path
...
def _process_rss_bytes() -> int:
    if os.name == "nt":
        ...
        if psapi.GetProcessMemoryInfo(kernel32.GetCurrentProcess(), ctypes.byref(counters), counters.cb) == 0:
            return 0
        return int(counters.WorkingSetSize)
    try:
        page_count = int(Path("/proc/self/statm").read_text(encoding="utf-8").split()[1])
    except (FileNotFoundError, IndexError, ValueError):
        return 0
    return page_count * int(os.sysconf("SC_PAGE_SIZE"))


def _make_memory_telemetry_callback(base_callback: StepCallback, telemetry_file: TextIO) -> StepCallback:
    previous_step_time = time.perf_counter()
    best_loss = float("inf")

    def record_step(step: int, theta: NDArray[np.float64], loss: float) -> None:
        nonlocal best_loss, previous_step_time
        base_callback(step, theta, loss)
        if step == 0:
            previous_step_time = time.perf_counter()
            return
            ...
            json.dumps(
                {
                    ...
                    "elapsed_step_seconds": float(current_time - previous_step_time),
                    "process_rss_bytes": _process_rss_bytes(),
                    "available_physical_memory_bytes": _available_physical_memory_bytes(),
                },
                separators=(",", ":"),
            )
            + "\n"
...
def main() -> None:
    ...
    if args.gradient_backend == "jax" and cfg.loss_function != "heisenberg_pauli":
        raise ValueError(
            "--gradient-backend jax currently supports only the heisenberg_pauli loss function"
        )
    ...
    telemetry_file: TextIO | None = None
    if args.gradient_backend == "jax" and args.memory_telemetry_path is not None:
        telemetry_file = args.memory_telemetry_path.open("a", encoding="utf-8")
    ...
    try:
        if args.init == "warmstart":
            ...
            if telemetry_file is not None:
                record_parameter_step = _make_memory_telemetry_callback(record_parameter_step, telemetry_file)
            result = adam_optimize(
                ...
                jax_memory_mode=args.jax_memory_mode,
            )
