"""Create a mixed-edge to Heisenberg warmstart success experiment."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create a mixed-edge warmstart Heisenberg success experiment.")
...
def main() -> None:
    ...
    stage1_loss = "edge_quantum_channel"
    stage1_superoperator = "superoperator_from_mix"
    stage2_loss = "heisenberg_pauli"
    resolve_loss_function_spec(stage1_loss)
    resolve_loss_function_spec(stage2_loss)
    resolve_superop(stage1_superoperator)
    ...
    manifest: dict[str, Any] = {
        "schema_version": 1,
        "artifact_type": "mixed_warmstart_experiment",
        "timestamp": stamp,
        ...
        "stage1": {
            "name": "mixed_edge",
            "loss_function": stage1_loss,
            "superoperator": stage1_superoperator,
            "iterations": mixed_iterations,
            "lr": mixed_lr,
        },
        "stage2": {
            "name": "heisenberg_warmstart",
            "loss_function": stage2_loss,
            "superoperator": None,
    ...
    print(f"Created mixed warmstart experiment: {experiment_root}")
    print(f"block_index = {args.block_index}")
    print(f"lightcone_size = {len(context.lightcone_qubits)}")
    print(f"stage1 = {stage1_loss}:{stage1_superoperator}, iterations={mixed_iterations}, lr={mixed_lr}")
    print(f"stage2 = {stage2_loss}, iterations={heisenberg_iterations}, lr={heisenberg_lr}")
    print(f"success_threshold = {float(args.success_threshold)}")
