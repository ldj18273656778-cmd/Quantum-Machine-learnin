"""Run one batch of random-initialization gradient statistics."""
...
from task2_code_auto.hpc_parallel_training.hpc_block_flow import PRESETS, atomic_savez, atomic_write_json
from task2_code_auto.module_e_training import build_target_objective_context
from task2_code_auto.gradient_analysis.gradient_statistics_core import compute_gradient_statistics_for_batch


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run one random-initialization gradient statistics batch.")
...
def _load_manifest(experiment_root: Path) -> dict[str, Any]:
    ...
    if data.get("artifact_type") != "random_initial_gradient_statistics_experiment":
...
def main() -> None:
    ...
    stats = compute_gradient_statistics_for_batch(initial_params, context, modes, fd_eps)
    ...
    atomic_savez(
        ...
        grad_sq_norms=stats["grad_sq_norms"],
        normalized_grad_sq_norms=stats["normalized_grad_sq_norms"],
    )
    payload = {
        ...
        "elapsed_seconds_by_mode": {str(mode["label"]): float(stats["elapsed_seconds_by_mode"][idx]) for idx, mode in enumerate(modes)},
    }
