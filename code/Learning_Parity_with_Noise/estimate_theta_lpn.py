"""Estimate original ISQNN theta parameters from the LPN training dataset."""
...
def rows_to_bitstrings(x: np.ndarray) -> np.ndarray:
    """Convert a binary matrix X into a 1D array of bitstrings."""
    x_arr = np.asarray(x, dtype=np.uint8)
    if x_arr.ndim != 2:
        raise ValueError(f"X must be a 2D matrix, got shape={x_arr.shape}.")
    if np.any((x_arr != 0) & (x_arr != 1)):
        raise ValueError("X must contain only 0/1 values.")
    return np.asarray(["".join(row.astype(str).tolist()) for row in x_arr], dtype=str)


def estimate_theta_from_training_data(
    X_train: np.ndarray,
    Y_train: np.ndarray,
    n1: int,
    m: int,
) -> dict[str, Any]:
    """Estimate all original ISQNN theta parameters from training samples."""
...
def save_theta_estimates(estimates: dict[str, Any], output_path: Path) -> None:
    """Save theta estimates to an npz file."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    np.savez(output_path, **estimates)


def print_estimation_summary(estimates: dict[str, Any], output_path: Path) -> None:
    """Print a concise summary of theta estimation results."""
    theta = np.asarray(estimates["theta_hat_flat_rad"])
    failed = np.asarray(estimates["failed_bits_0based"])

    print("Original ISQNN theta estimation from LPN training data")
...
def main() -> None:
    ...
    estimates = estimate_theta_from_training_data(
    ...
    estimates.update(
        {
            ...
            "theta_path": str(config.THETA_PATH),
            "n_x": int(dataset["n_x"]),
            "n_y": int(dataset["n_y"]),
    ...
    save_theta_estimates(estimates, config.THETA_PATH)
    print_estimation_summary(estimates, config.THETA_PATH)
