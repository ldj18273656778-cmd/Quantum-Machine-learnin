...
def DQNN_generate_y_rx(bitstring, n1, m, theta_list, rx_angle=0.0):
    try:
        ...
        cirq = None
        print(f"Warning: cirq import failed in DQNN_generate_y_rx: {e}")
    ...
    # Rx rotation (mixes information across CZ barriers)
    if rx_angle != 0.0:
        for i in range(m):
            full_circuit.append(cirq.rx(rx_angle)(qubits[i]))
    ...
    for block in range(1, n1):
        for i in range(m):
            ...
            if x_bit == "0":
                ...
                result = sim.simulate(
                    h_circuit, initial_state=state, qubit_order=qubits
                )
                ...
                if meas == 1:
                    ...
                    result = sim.simulate(
                        x_circuit, initial_state=state, qubit_order=qubits
                    )
                    state = result.final_state_vector
                ...
                h_circuit.append(
                    cirq.H(qubits[i])
                )  # 不能使用h_circuit.append(cirq.measure(cirq.X(qubits[i]), key='meas'))对X进行测量，因为Cirq不支持直接测量X基。正确的做法是先应用H门将X基转换为Z基，然后测量Z基。
                h_circuit.append(
                    cirq.measure(qubits[i], key="meas")
                )  # 测量X并记录结果至meas
                result = sim.simulate(
                    h_circuit, initial_state=state, qubit_order=qubits
                )
                ...
                # 重置为 |0>
                reset_circuit = cirq.Circuit(cirq.ResetChannel().on(qubits[i]))
                result = sim.simulate(
                    reset_circuit, initial_state=state, qubit_order=qubits
                )
                state = result.final_state_vector
        ...
        if block <= n1 - 1:  # 更正为小于等于 n1 - 1，确保最后一个block也处理
            ...
            if rx_angle != 0.0:
                for i in range(m):
                    block_circuit.append(cirq.rx(rx_angle)(qubits[i]))
                    full_circuit.append(cirq.rx(rx_angle)(qubits[i]))
            ...
            result = sim.simulate(
                block_circuit, initial_state=state, qubit_order=qubits
            )
            state = result.final_state_vector
    ...
    result = sim.simulate(
        final_measure_circuit, initial_state=state, qubit_order=qubits
    )
...
if __name__ == "__main__":
    ...
    n = n1 * m
    theta_test = [
        random.uniform(0, 1) * np.pi for _ in range(n)
    ]  # 随机生成theta参数列表

    circuit, y = DQNN_generate_y_rx(bitstring, n1, m, theta_test)
